//package advanced_techniques.meet_in_the_middle;
...
public class Solution {
    ...
    private void iterateSubsets(IntConsumer sumConsumer, int sumAccum, int[] arr, int idx){
        ...
        boolean canAddCurrElem = sumAccum <= target-arr[idx]; // or equivalently sumAccum + arr[idx] <= target (checking it that way to prevent overflows).
    ...
    private void solve(){
        ...
        if (totalSmallerThanTargetSum < target){
            ...
            // sorting the list form bigger to smaller, ensures that the first half has largest half elements, since we don't add to hashmap any sums that cross target, the hashmap storage consumes lesser space.
